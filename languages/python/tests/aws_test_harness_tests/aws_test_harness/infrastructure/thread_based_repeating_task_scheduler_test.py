...
def test_executes_scheduled_task(logger: Logger) -> None:
    ...
    wait_for_value_matching(
        lambda: task,
        f'task mock to have been called',
        lambda the_task: cast(Mock, the_task).called
    )


def test_repeatedly_executes_scheduled_task(logger: Logger) -> None:
    repeating_task_scheduler = ThreadBasedRepeatingTaskScheduler(logger)
    task = Mock()
    ...
    wait_for_value_matching(
        lambda: task,
        f'task mock to have been called more than twice',
        lambda the_task: cast(Mock, the_task).call_count > 2
    )


def test_throws_exception_if_asked_to_schedule_task_when_one_is_already_scheduled(logger: Logger) -> None:
    repeating_task_scheduler = ThreadBasedRepeatingTaskScheduler(logger)
    task1 = Mock()
    task2 = Mock()
...
def test_stops_scheduling_task_when_instructed(logger: Logger) -> None:
    repeating_task_scheduler = ThreadBasedRepeatingTaskScheduler(logger)
    task = Mock()
...
def test_does_not_throw_exception_if_asked_to_reset_schedule_before_a_task_is_scheduled(logger: Logger) -> None:
    repeating_task_scheduler = ThreadBasedRepeatingTaskScheduler(logger)
    repeating_task_scheduler.reset_schedule()


def test_can_schedule_again_after_resetting_schedule(logger: Logger) -> None:
    repeating_task_scheduler = ThreadBasedRepeatingTaskScheduler(logger)
    first_task = Mock()
    ...
    wait_for_value_matching(
        lambda: first_task,
        f'first task mock to have been called',
        lambda the_task: cast(Mock, the_task).called
    )
    ...
    wait_for_value_matching(
        lambda: second_task,
        f'second task mock to have been called',
        lambda the_task: cast(Mock, the_task).call_count > 0
    )


def test_continues_scheduling_task_after_task_throws_exception(logger: Logger) -> None:
    repeating_task_scheduler = ThreadBasedRepeatingTaskScheduler(logger)
    task = Mock()
    ...
    wait_for_value_matching(
        lambda: task,
        f'task mock to have been called multiple times',
        lambda the_task: cast(Mock, the_task).call_count > 1
    )


def test_indicates_that_a_task_has_been_scheduled(logger: Logger) -> None:
    repeating_task_scheduler = ThreadBasedRepeatingTaskScheduler(logger)
    task = Mock()
...
def test_indicates_that_a_task_has_not_been_scheduled_after_resetting_schedule(logger: Logger) -> None:
    repeating_task_scheduler = ThreadBasedRepeatingTaskScheduler(logger)
    task = Mock()
    repeating_task_scheduler.schedule(task)
    assert repeating_task_scheduler.scheduled() is True
