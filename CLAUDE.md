...
- **"Tidy first" refactoring** - When anticipating a feature addition and finding the code structure inadequate, make structural changes without behavioral changes first, then add the new behavior. This follows Kent Beck's principle of "make the change easy, then make the easy change"
...
- **Outside-In API Design Through TDD** - Design APIs by first writing the calling code you wish existed, let it fail, then implement only what's needed to make it pass. This drives intuitive API design from the caller's perspective, prevents over-engineering and feature creep, ensures APIs solve actual problems not imagined ones, and creates clear development progression through concrete failures. Use when designing new interfaces or extending existing ones, especially in test-driven development scenarios.
...
- **Consistent Architectural Patterns** - When extending existing systems, align new functionality with established patterns rather than creating novel approaches. This reduces cognitive load for developers, maintains system coherence, leverages existing infrastructure and conventions, and makes the codebase more predictable and maintainable. Use when adding new resource types or capabilities to an existing framework that already has established patterns for similar functionality.
- **Root Cause Analysis Through Error Message Archaeology** - When debugging complex systems, trace error messages back through all layers of execution to understand the true cause rather than treating symptoms. This prevents fixing the wrong thing, builds deeper system understanding, leads to more robust solutions, and saves time by addressing root causes directly. Use when encountering failures in complex, multi-layered systems with message passing, queuing, or distributed components.
