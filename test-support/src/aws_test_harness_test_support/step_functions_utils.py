...
def start_state_machine_execution(execution_input: dict[str, str], state_machine_arn: str,
                                  boto_session: Session) -> StateMachineTestExecution:
    step_functions_client: SFNClient = get_state_machine_client(boto_session)
    ...
    return StateMachineTestExecution(step_functions_client, start_execution_result['executionArn'])
...
def execute_state_machine(state_machine_arn: str, boto_session: Session,
                          execution_input: Dict[str, Any]) -> DescribeExecutionOutputTypeDef:
    execution = start_state_machine_execution(execution_input, state_machine_arn, boto_session)

    return wait_for_state_machine_execution_completion(execution.arn, get_state_machine_client(boto_session))
...
def get_state_machine_client(boto_session: Session) -> Any:
